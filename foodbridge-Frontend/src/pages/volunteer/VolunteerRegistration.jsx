...
import Volunteerlogin from './Volnteerlogin';
...
function VolunteerRegistration() {
    ...
    let [vehicle,setVehicle] = useState("none")
    ...
    const handleSignup = async (e) => { //***MAIN PART: handle signup when form is filled */
        ...
        e.preventDefault()
        try {
         const result = await axios.post(serverUrl + '/api/auth/volunteer/registration',{ //calls our backend sign-up api n sends form data
            name,email,password,city,cnic,vehicle
         },{withCredentials:true})
           // getCurrentUser()
            navigate("/volunteerpanel")   //navigate to volunteer panel after successful signup
    ...
    <div className='max-w-[600px] w-[90%] h-[500px] bg-[#00000025] border-[1px] border-[#96969635] backdrop:blur-2xl rounded-lg shadow-lg flex items-center justify-center '>
      ...
      {/* will add G-Authentication here! */}
            ...
            <div className='w-[90%] h-[400px] flex flex-col items-center justify-center gap-[15px]  relative'>
                ...
                <input type="text" className='w-[100%] h-[50px] border-[2px] border-[#96969635] backdrop:blur-sm rounded-lg shadow-lg bg-transparent placeholder-[#ffffffc7] px-[20px] font-semibold' placeholder='City' required onChange={(e)=>setCity(e.target.value)} value={city}/>
                <input type="text" className='w-[100%] h-[50px] border-[2px] border-[#96969635] backdrop:blur-sm rounded-lg shadow-lg bg-transparent placeholder-[#ffffffc7] px-[20px] font-semibold' placeholder='CNIC' required onChange={(e)=>setCnic(e.target.value)} value={cnic}/>
                <select className='w-[100%] h-[50px] border-[2px] border-[#96969635] backdrop:blur-sm rounded-lg shadow-lg bg-transparent placeholder-[#ffffffc7] px-[20px] font-semibold' required onChange={(e)=>setVehicle(e.target.value)} value={vehicle}>
                    <option value="none" className='bg-[#141414]'>None</option>
                    <option value="car" className='bg-[#141414]'>Car</option>
                    <option value="bike" className='bg-[#141414]'>Bike</option>
                </select>
                ...
                <p className='flex gap-[10px]'>You have any account? <span className='text-[#5555f6cf] text-[17px] font-semibold cursor-pointer' onClick={()=>navigate("/volunteerlogin")}>Login</span></p>
            </div>
        </form>
    </div>
    </div>
